...
class sportCars:public Car{
  ...
  public:
  sportCars( const string &model,const string &brand){
    this->model = model;
    this->brand = brand;
    currSpeed = 0;
    currGear = 0;
  ...
  void startEngine() override {
     if(isEngineOn)
       cout << "engine is already satarted"<<endl;
     else
       isEngineOn = true;
  ...
  void shiftGear(int gear) override{
      ...
      currGear += gear;
      cout << "gear shifted successfully"<<endl;
  }
  ...
  void applyBreak() override{
    ...
    cout << "car is slowing down with a speed of : -20km/hr" << endl;
  }
...
int main(){
  Car *car1 = new sportCars("Ford", "Mustang");
  car1->startEngine();
  car1->shiftGear(1);
  car1->accerelate();
  car1->shiftGear(2);
